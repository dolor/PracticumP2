...
/**
 * P2 prac wk3. Uitslag. Model voor uitslag stemming.
 * 
...
public class Uitslag extends Observable {
	...
	/**
	 * Levert het aantal tot dusverre uitgebrachte stemmen op de aangegeven
	 * partij op.
	 * 
	 * @require <code>this.getPartijen().contains(partij)</code>
	 * @param partij
	 *            de partij waarvan het aantal stemmen bepaald wordt
	 */
	...
	/**
	 * Voegt een partij toe aan de bestaande partijen. Het aantal stemmen is
	 * aanvankelijk 0. Als de partij al bestond, gebeurt er niets.
	 * 
	...
	public void voegPartijToe(String partij) {
		...
		notifyObservers(stemmen);
	}

	/**
	 * Telt 1 op bij het aantal stemmen van een partij. Als de partij niet
	 * bestaat, gebeurt er niets.
	 * 
	 * @param partij
	 *            de partij die een stem erbij krijgt
	 * @ensure als <code>this.getPartijen().contains(partij)</code> dan
	 *         <code>this.getStemmenOp(partij) == old.getStemmenOp(partij)+1</code>
	 */
